...
class CollectionDetails(restful.Resource):
    ...
    def get_data():
            ...
            for db in db_names:
                     __str = "conn.{0}.collection_names()".format(db)
                     j = eval(__str)
                     j.pop(0)
                     for collection in j:
                            __collection_name  =  "connection.{0}.{1}".format(db, collection)
                            __collection = eval(__collection_name)
                            collections.append(__collection)
                            print list(__collection.find().limit(1))
            #collections = [news_collection_cric, news_collection_bask, news_collection_f1rc, news_collection_ftbl, news_collection_tenn]
	                    #number_of_entries = []
	                    #single_entry_from_collection = []
                            #for coll in __collection:
		            number_of_entries.append(__collection.count())
		            for entry in __collection.find(projection={'_id':False, 'news':False}).sort("publish_epoch", -1).limit(1):
