...
class GetPlayers:
        ...
        'age' : {'index': 'not_analyzed', 'type': 'string'}
                ...
                if not ES_CLIENT.indices.exists("cricket_players"):
                        self.prep_teams_index()
                        self.index_data()

                if renew_indexes:
                        ES_CLIENT.indices.delete(index="cricket_players")
                        self.prep_teams_index()
                        self.index_data()


        def prep_teams_index(self):
                ES_CLIENT.indices.create(index="cricket_players", body=self.settings)
                ES_CLIENT.indices.put_mapping(index="cricket_players", doc_type="cricket_players", body = {"cricket_players": self.mappings })
                a = "Mappings updated for  {0}".format("cricket_players")
        ...
        def index_data(self):
                ...
                for player in self.cricket_players.find(projection={'_id':False}):
                        print ES_CLIENT.index(index='cricket_players',doc_type='cricket_players',body=player)
