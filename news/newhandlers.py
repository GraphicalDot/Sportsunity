...
class NewsRequestHandler(tornado.web.RequestHandler):
    ...
    def get_news_with_search(self, news_type, timestamp, direction):
        ...
        result = sorted(result, key=operator.itemgetter('publish_epoch'), reverse=True)
        print result
        if not timestamp:
...
class GetMixedNews(NewsRequestHandler):
    ...
    def __init__(self):
            super(NewsRequestHandler, self).__init__()
    """
    ...
    def get(self, *args, **kwargs):
        ...
        try:
            ...
            print terminal.on_red("Time taken to execute the reuest %s"%(time.time() - start))
            self.finish()
