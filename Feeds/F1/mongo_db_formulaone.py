...
class FormFeedMongo(object):

        @staticmethod
        def if_news_exists(news_id, news_link):
                if news_collection_f1rc.find_one({"news_id": news_id, "news_link": news_link}):
                        return True
        ...
        def insert_news(news_dict):
                news_collection_f1rc.insert(news_dict)
                """
                bulk = news_collection_f1rc.initialize_unordered_bulk_op()
