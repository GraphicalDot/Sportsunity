...
class CricketCbuz:
        ...
        def full_news(self):
			...
			summarization_instance = ShortNews()
                        ...
                        try:
                                news_dict.update({"website": "www.cricbuzz.com", "summary":summarization_instance.summarization(full_text),\
					...
					time.mktime(time.localtime()),'type':'cricket','favicon':'http://www.cricbuzz.com/favicon.ico'})
                        except:
                                news_dict.update({"website": "www.cricbuzz.com", "summary":summary,"custom_summary": summary, "news":\
                                        full_text, "image_link":image_link,'gmt_epoch':gmt_epoch,'publish_epoch':publish_epoch, "day": day, "month":\
                                        month, "year": year,'ldpi': all_formats_image['ldpi'],'mdpi': all_formats_image['mdpi'],\
                                        'hdpi': all_formats_image['hdpi'],"time_of_storing":time.mktime(time.localtime()),'type':\
					'cricket','favicon':'http://www.cricbuzz.com/favicon.ico'})
...
if __name__ == '__main__':
    obj = CricketCbuz(CBUZ_CRIC_FEED)
    obj.run()
    #obj.rss_feeds(CBUZ_CRIC_FEED)
    #obj.checking()
    #obj.full_news()
