...
class Cricket_CBUZ:
    ...
    def rss_feeds(self,CBUZ_CRIC_FEED):
        list_of_links = list()
        self.list_of_links = list_of_links
        self.d = feedparser.parse(CBUZ_CRIC_FEED)
        self.details = self.d.entries
        for entry in self.details:
            self.list_of_links.append(entry['feedburner_origlink'])
    ...
    def full_news(self):
	    ...
	    image = article.top_image.get_src()
            ...
            _dict = {"website":"CBUZ_CRIC_FEED","news_id":val,"summary":summary,"publish_date":publish_date,"news":full_text,"title":title,"image":image,'ldpi':all_formats_image['ldpi'],'mdpi':all_formats_image['mdpi'],'hdpi':all_formats_image['hdpi'],"time_of_storing":time.mktime(time.localtime())}
